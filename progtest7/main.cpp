...
template <typename T_, typename Compare_ = less<typename T_::value_type>>
class CIndex
{
public:
    CIndex() {};
    CIndex(const T_ & in, const Compare_ & compare) : cmp(compare), data(in), pos(0) {};
    CIndex(const T_ & other) { data = T_(other); }
    ~CIndex() = default;
    set<size_t> search (const T_ & key)
    {
        set <size_t> result;
        pos = 0;
        for (auto i = data.begin(); i != data.end(); i++, pos++)
        {
            bool isValue = true;
            auto tmp = i;
            auto keyIt = key.begin();
            for ( ; keyIt != key.end() && tmp != data.end(); tmp++, keyIt++)
            {
                if (!(!cmp(*tmp, *keyIt) && !cmp(*keyIt, *tmp)))
                {
                    isValue = false;
                    break;
                }
